# Replace DDPM with FM in MAR

In this repository, we attempt to replace the diffusion loss in MAR with the flow matching loss from [SiT](https://arxiv.org/abs/2401.08740).

Notice: We use ODE by default, so temperature has no effect on the sampling results.

| MAR Model | Loss | Train Epochs | FID-50K | Inception Score |
|-----------|------|--------------|---------|-----------------|
| MAR-L | Diffusion Loss | 50 | 13.17 | 97.04 |
| MAR-L | Flow Matching Loss | 50 | 15.28 | 85.73 |
